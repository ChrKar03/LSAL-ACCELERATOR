...
void lsal_compute_matrices_p(char *q, char *d, size_t *max_idx, int *similarity, char *direction, size_t N, size_t M) {
    int max_similarity = 0;
    *max_idx = 0;

    for (int round = 0; round < N + M - 1; round++) {
        int start = max(0, round - N + 1);
        int end = min(round, M - 1);
...
int main(int argc, char **argv) {
    ...
    for (size_t i = 0; i < num_iter; i++)
        ...
        lsal_compute_matrices_p(q, d, &max_idx, similarity, direction, qlen, dlen);
